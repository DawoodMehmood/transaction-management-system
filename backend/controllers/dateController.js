...
exports.addOrUpdateDates = async (req, res) => {
    const { state_id, dates, transaction_id, stage_id, created_by, transaction_type } = req.body;
  
    if (!state_id || !created_by || !transaction_id || !stage_id || !Array.isArray(dates) || dates.length === 0 || !transaction_type) {
    ...
    try {
      ...
      const queries = dates.map(({ date_id, date_name, date_value }) => ({
        text: `
          INSERT INTO tkg.dates (state_id, date_id, date_name, entered_date, created_date, created_by, transaction_id, stage_id, updated_date, updated_by, transaction_type) 
          VALUES ($1, $2, $3, $4::DATE, CURRENT_TIMESTAMP, $5, $6, $7, CURRENT_TIMESTAMP, $5, $8)
          ON CONFLICT (state_id, date_name, transaction_id, stage_id, transaction_type) 
          DO UPDATE SET 
              entered_date = $4::DATE,
              updated_date = NOW(), 
              updated_by = $5 
          RETURNING *;
        `,
        values: [state_id, date_id, date_name, date_value, created_by, transaction_id, stage_id, transaction_type],
      }));
      ...
      const fetchTasksQuery = `
        ...
        WHERE td.transaction_id = $1 AND td.task_days IS NULL AND td.stage_id = $2 AND td.transaction_type = $3
        ORDER BY td.task_id, td.transaction_detail_id;
      `;
      const tasksWithNullTaskDays = (await pool.query(fetchTasksQuery, [transaction_id, stage_id, transaction_type])).rows;
        ...
        if (taskConfig.is_repeatable && assignedDate) {
          ...
          for (let i = 0; i < taskGroup.length; i++) {
            ...
            await pool.query(
              ...
              SET task_days = $1
              WHERE transaction_detail_id = $2 AND transaction_id = $3 AND transaction_type = $4;
              `,
              [newTaskDays, task.transaction_detail_id, transaction_id, transaction_type]
            );
          }
        }
      }
  ...
  exports.getTransactionDates = async (req, res) => {
    ...
    if (req.user && req.user.role !== 'superadmin' && req.user.allowedStates && req.user.allowedStates.length > 0) {
      allowedStates = req.user.allowedStates;
    }
    ...
    try {
      ...
      // If allowedStates is not empty, add filter condition
      if (allowedStates.length > 0) {
        query += ` AND d.state_id = ANY($2)`;
        params.push(allowedStates);
      }
  
      query += ` ORDER BY d.stage_id, d.date_name;`;
      ...
      res.status(200).json({
        ...
        transaction_id,
        stages: Object.values(groupedDates),
      });
    } catch (error) {
  ...
  exports.getTransactionDatesByStage = async (req, res) => {
    ...
    // If the user is not a superadmin, get their allowed states.
    let allowedStates = [];
    if (
      req.user &&
    ...
    try {
      ...
      `;
      const params = [transaction_id, stage_id];
  
      // If allowedStates exists, filter to only those states.
      if (allowedStates.length > 0) {
        query += ` AND d.state_id = ANY($3)`;
        params.push(allowedStates);
      }
  
      query += ` ORDER BY d.date_name;`;
      ...
      res.status(200).json({
        ...
        transaction_id,
        stage_id,
        stage_name: result.rows[0].stage_name, // Assumes all rows have the same stage_name.
        dates,
      });
    } catch (error) {
...
exports.getAllTransactionsCalendar = async (req, res) => {
  try {
    ...
    const { state, transactionType } = req.query;

    // Build the base query with date binding and state filter.
    let query = `
      SELECT 
          t.transaction_id, 
    ...
    // If a transactionType is provided (non-empty), add the condition.
    if (transactionType && transactionType.trim() !== "") {
      query += ` AND td.transaction_type = $2`;
      params.push(transactionType);
    }

    query += ` ORDER BY task_due_date;`;
    ...
    const groupedTransactions = result.rows.reduce((acc, row) => {
      const { transaction_id, transaction_name, address1, address2, city, state, zip, list_price, state_name, transaction_type } = row;
      const { date_name, task_due_date, task_id, task_name, task_status, stage_id, transaction_detail_id, is_skipped, skip_reason } = row;
