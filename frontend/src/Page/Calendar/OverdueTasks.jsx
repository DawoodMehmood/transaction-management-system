...
export const OverdueTasks = ({ setupdatedLoading }) => {
    ...
    const fetchOverdueTasks = async () => {
      try {
        ...
        if (data && data.transactions) {
          ...
          // Filter tasks that are not in today, this week, or this month
          const tasksByStage = data.transactions
            ...
            .filter((task) => {
              // Check if task is overdue (past entered date and not in today, this week, or this month)
              const isOverdue =
                task.enteredDate &&
                task.enteredDate < today && // Task entered in the past
                !(
                ...
                task.task_status === 'Open'; // Task must be open
              return isOverdue;
            })
            .reduce((acc, task) => {
