...
CREATE TABLE
    tkg.transaction_detail (
        ...
        skipped_date date NULL, -- When the task was skipped
...
CREATE TABLE
    tkg.user_states (
        ...
        state varchar(2) NOT NULL,
        CONSTRAINT user_states_pkey PRIMARY KEY (user_id, state),
        CONSTRAINT fk_user FOREIGN KEY (user_id) REFERENCES tkg.users (user_id) ON DELETE CASCADE,
        CONSTRAINT fk_state FOREIGN KEY (state) REFERENCES tkg.state (state) ON DELETE CASCADE
    );
...
ALTER TABLE tkg.dates ADD CONSTRAINT fk_transaction FOREIGN KEY (transaction_id) REFERENCES tkg."transaction" (transaction_id) on DELETE CASCADE;
